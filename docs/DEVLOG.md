...
## 2026-07-02 — 추천 매칭 설명 /recommend/explain (교육·공고 공용)

### 목적
- E-013(이력서+레벨테스트 기반 추천)의 "왜 이게 당신에게 맞는지" 서술 생성. 랭킹 점수는 Spring이 결정론 휴리스틱으로 산출하고, 설명 문장만 이 엔드포인트가 담당.

### 설계 — base + prefill, 배치 1회 호출
- followup 계열과 동일: base 모델(use_adapter=False) + 한국어 prefill(EXPLAIN_PREFILL) + parse_json_lenient.
- items 전체를 한 번의 generate로 처리 → target_id별 reason을 배열로 반환. 후보 N개를 N번 호출하지 않아 trycloudflare ~100초 타임아웃 회피.
- 응답: {ok, target_type, reasons:[{target_id, reason}]}. target_id 키잉이라 Spring이 순서 무관하게 매핑.

### target_type 공용 (교육·공고)
- 요청 target_type(COURSE/POSTING)을 응답에 그대로 되돌림 → 공고 담당 팀원이 POSTING으로 같은 엔드포인트 재사용. AI 설명 로직을 교육·공고에 하나로 통일.

### 환각 방지 / 폴백
- 프롬프트가 제공된 matched(걸린 매칭 근거)에만 기반하도록 강제("없는 내용 지어내지 말 것"). 프로필 원문이 아니라 구조화 신호(job_category/level/skills)만 전달.
- 파싱 실패 시 ok:false 반환 → Spring 측 규칙 문장 폴백과 짝. AI 엔드포인트가 없어도 /api/recommend/courses는 완전 동작.

### 원칙 / 검증
- 하이브리드 유지: 점수=Spring 휴리스틱(결정론), 서술만 LLM. 랭킹에 GPU 불필요 → 파드 off 상태에서도 추천 자체는 동작.
- 검증: 파드에서 2항목 배치 호출 → ok:true, target_id 2개 모두 matched 근거 기반 한국어 이유 생성 확인. (일지엔 근거 문장 원문·AI Hub 원본 미포함.)


---

## 2026-07-07 — /education/quiz/stream: 퀴즈 SSE 스트리밍 추가

### 목적
기존 /education/quiz(블로킹)는 run_llm으로 최대 3072토큰을 일괄 생성한 뒤 응답한다.
문항 수가 많으면 생성이 길어져 Cloudflare 524(~100s) 타임아웃 위험이 있고, 사용자는
완료 전까지 아무 피드백도 받지 못한다. 이미 evaluate/lesson/chat은 SSE 스트리밍을
제공하는데 education 도메인에서 lesson은 스트리밍, quiz만 비스트리밍이라 그 공백을
메운다. (기존 엔드포인트 유지, 스트리밍 변형을 가산)

### 구현
- _eval_stream_gen 패턴 준용. quiz는 evaluate처럼 "전량 생성 → JSON 파싱 → 구조화"
  흐름이라 가장 잘 맞는다.
- TextIteratorStreamer + 스레드 generate. GEN_LOCK으로 GPU 직렬화(비스트리밍과 동일 락).
- quiz는 항상 base 모델(use_adapter=False, disable_adapter). 어댑터는 채점 전용.
- 샘플링은 do_sample=True + temperature=0.8 / top_p=0.9로, 비스트리밍 run_llm의 기본
  샘플링값과 동일하게 맞춰 출력 특성 일치.
- quiz_prompt / QUIZ_PREFILL / _build_quiz_items / parse_json_lenient 재사용(중복 없음).
- SSE 계약: 생성 중 token 이벤트, 종료 시 done(ok, topic, count, quiz), 예외 시 error.
  evaluate/stream과 동일 봉투.
- 라우트는 파일 끝에 append(FastAPI는 등록 순서 무관). 비스트리밍 /education/quiz는
  손대지 않음(비파괴/가산).

### 검증
- 재시작 전 ast.parse 문법 검사(실패 시 백업 자동 롤백 가드).
- 재시작 후: quiz 라우트 2개 등록 확인, /health ready:true.
- 스모크 + 3회 연속 실전 테스트(토픽·난이도·문항 수 교차):
  - RUN1 HTTP status codes / n3 / 하 → PASS (token 179)
  - RUN2 SQL JOIN types / n2 / 상 → PASS (token 300)
  - RUN3 Python list vs tuple / n4 / 중 → PASS (token 1304)
  - 3회 모두 token>0 · done.ok=true · errors=0. 토큰 수가 문항 수에 비례 증가 →
    연속 요청에서도 GEN_LOCK 직렬화 정상.

### 트러블슈팅
- 최초 스모크가 빈 응답(token 0/done 없음). 원인은 스트리밍 코드가 아니라 라이브
  서버의 X-API-Key 인증. 헤더 없는 요청은 엔드포인트 도달 전 미들웨어에서 거부됨
  ("인증 실패: X-API-Key ..."). .api_key에서 키를 읽어 헤더를 실으니 즉시 정상.
  → 백엔드(Spring) aiRestClient도 이 헤더 필수(후속 점검 대상).
- 관찰: _build_quiz_items 품질 필터가 "고유 오답 3개 미만" 문항을 버린다. 난이도 하/상
  에서 base 모델이 정답과 겹치는 부실 오답을 낼 때 채택 문항이 줄어 count < 요청 n 발생
  (n3→1, n2→1). n4(중)는 4개 정확. 비스트리밍 엔드포인트와 공유하는 기존 동작(스트리밍이
  만든 문제 아님). 별도 개선 과제로 기록(오답 품질 프롬프트 강화 또는 부족분 재생성).

### 영향 / 다음 단계
- education quiz가 블로킹·SSE 두 변형을 모두 제공. 프론트는 실시간 UX와 524 회피를 위해
  /education/quiz/stream으로 전환 가능. 파괴적 변경 없음.
- 후속: (1) Spring aiRestClient의 X-API-Key 주입 확인, (2) 오답 품질/부족분 재생성 개선.
...
## 2026-07-08 — 추론 속도 튜닝(reason_budget) + 채점 어댑터 OFF(속도)

### 목적
Colab(L4)/팟(RTX 4090)에서 추론이 느리다는 문제를 런타임 측정으로 원인 특정하고 개선한다.

### 배경 측정 (런타임 실측)
- 질문 생성(/interview/generate, adapter=False): 563tok / 39.4s = 14.3 tok/s
- 채점(/interview/evaluate, adapter=True/ko): 800tok / 97.5s = 8.2 tok/s
- 판정: tok/s는 4090·L4 모두 정상권(L4도 14.3로 4090급). 느림의 주범은 (1) 사고(<thought>) 토큰 과다,
  (2) 채점의 어댑터가 tok/s를 8.2로 저하 — 두 가지. (BUDGET_FORCE=True이나 REASON_BUDGET=1000이
  헐거워 질문 생성 563·채점 800 모두 강제 종료 미발동.)

### 구현 1 — 엔드포인트별 사고 예산(reason_budget)
- run_llm에 reason_budget=None 인자 추가. 지정 시 그 값, 미지정 시 전역 REASON_BUDGET(하위 호환).
- 질문 생성·꼬리질문에 reason_budget=350 지정(어댑터 없음·질문 품질에 350이면 충분). 채점·리포트는 미지정.
- 결과: 질문 생성 437tok / 16.7s = 26.1 tok/s [FORCED] — 39.4s→16.7s(약 2.4배), 질문 품질 유지.

### 구현 2 — 채점 어댑터 OFF (속도 목적)
- 어댑터 사용처는 채점 2곳뿐: /interview/evaluate(블로킹), /interview/evaluate/stream(SSE).
  (질문생성·이력서·개념설명·챗봇·quiz·레벨테스트는 이미 base/disable_adapter.)
- 두 곳 use_adapter=(lang=="ko") → use_adapter=False. 어댑터 로딩(PeftModel.from_pretrained)은 유지
  → health.adapter_loaded=true, 발표 후 재학습·재도입 대비.
- 결과: 채점 927tok / 18.8s = 49.4 tok/s (97s→19s, 약 6배 tok/s). 한국어 피드백 품질 양호.

### 조사 기록 — "5축 척도가 깨졌다"는 오진 정정 (중요)
- 초기에 채점 scores {technical_accuracy:4, specificity:3, ...} 를 보고 "0~100 지시인데 1~5로 왜곡됐다,
  어댑터가 원인"이라 판단했으나 오진이었음.
- 실측 반증: 어댑터 ON/OFF, 팟/Colab 무관하게 한국어 scores가 0~5로 나옴. 코드 확인 결과
  clamp_scores가 5축을 0~5로 정규화(min(5, v))하도록 되어 있음 — 이는 "원래 0~100 설계가 잘 안 되어
  0~5로 변경"한 의도된 설계(프롬프트 주석: "This 1-5 scale is separate from the 0-100 axis scores").
  즉 scores 0~5는 버그가 아니라 정상. display_scores/overall/STAR(×20)도 이 0~5 파이프라인에 정합.
- 어댑터 실험에서 85/70이 나온 건 영어(lang=en) 경로였고, 한국어(0~5 파이프라인)에 잘못 적용해 착각한 것.
- 결론: 척도는 어댑터와 무관. 따라서 어댑터 OFF의 근거는 '척도'가 아니라 '속도'로 정정. 어댑터 OFF 결정 자체는 유지.

### 결정 / 다음 단계
- 어댑터 OFF 유지: 근거는 속도(8.2→49 tok/s). base 한국어 채점 품질 양호. 로딩은 유지(재도입 대비).
- 후속 과제: 어댑터 ON vs OFF의 채점 품질을 다양한 답변(우수/부실/빈/오답)으로 비교 후 재도입 여부 확정.
  어댑터 재학습(발표 후) 시 함께 판단.
- 사소: 채점 do_sample=False인데 temperature/top_p 잔존으로 경고(동작 무해) — 추후 정리.

## 2026-07-09 — 5축 채점 전환 (심화이해/depth 추가)
- 기존 4축(technical_accuracy/specificity/logic/communication)에 **depth(심화이해)** 축 추가 → 5축.
- 변경점: SCORE_KEYS·LABEL(ko "심화이해"/en "Depth")에 depth 추가, RUBRIC_EN을 "five axes"로 갱신(0~100 스케일 JSON 예시 2개에 depth 포함), 한국어 루브릭(train.rubric.jsonl 1행)에 심화이해 기준 반영.
- clamp_scores·fix_overall·report 집계는 SCORE_KEYS 순회 구조라 코드 수정 없이 자동 5축 확장. clamp는 0~100(max(0,min(100,v))), overall은 축 평균(1번 축 0이면 나머지 평균).
- 채점은 어댑터 미사용(base + 프롬프트). 어댑터 OFF 근거는 '속도'(8.2→49 tok/s)이며 척도와는 무관(척도는 어댑터 ON/OFF와 독립, 앞선 항목에서 정정됨). 어댑터 로딩은 재도입 대비 유지.
- 라이브 검증(/interview/evaluate, lang=ko, 0~100): 상세 답변 depth=70 / "잘 모르겠어요" depth=0 으로 변별 정상. overall 79 vs 12.
- 응답 계약 불변: {ok, evaluation:{scores(+depth), overall, display_scores, star, ...}}. display_scores(레이더용) 매핑은 미변경.
